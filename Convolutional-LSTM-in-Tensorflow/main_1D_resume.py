...
tf.app.flags.DEFINE_string('train_dir', './checkpoints/resume',
    ...
    for step in range(FLAGS.max_step):
      ...
      if step%4000 == 0:
        ...
        if match_mask:
            im_x = im_x[...,1]
            im_y = im_y[...,1]
        _plot_samples(dat[:,:FLAGS.seq_start,:,:].squeeze(), sample_dir+'resume_step_{}_past_t.png'.format(step))
        _plot_samples(im_x.squeeze(), sample_dir+'resume_step_{}_past.png'.format(step))
        _plot_samples(dat[:,FLAGS.seq_start:,:,:].squeeze(), sample_dir+'resume_step_{}_future_t.png'.format(step))
        _plot_samples(im_y.squeeze(), sample_dir+'resume_step_{}_future.png'.format(step))
...
def test(test_mode='anomaly', with_y=True):
  with tf.Graph().as_default():
    ...
    for step in range(FLAGS.max_step):
      ...
      #import IPython; IPython.embed()
      _plot_samples(np.vstack([x_t.squeeze(), y_t.squeeze()]), sample_dir+'resume_GT_{}.png'.format(step))
      _plot_samples(np.vstack([im_x.squeeze(), im_y.squeeze()]), sample_dir+'resume_pred_{}.png'.format(step))
      #print("loss", dloss)
      #import IPython; IPython.embed()
